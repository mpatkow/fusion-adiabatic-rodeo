...
plt.xlabel(r"adiabatic time ratio: $\frac{T_\mathrm{RA}}{T}$")
plt.ylabel(r"total time: $T$")
plt.yticks(np.linspace(0,1,ytickslen), np.linspace(0,int(np.max(y_values_unnorm[0])+2), ytickslen)) #, rotation='vertical')
#print(int(np.max(y_values_unnorm[0])))
#plt.yticks(np.linspace(0,1,8), np.linspace(0,78,8)) #, rotation='vertical')
plt.colorbar()
plt.show()
#plt.subplot(122)
