...
for datafilename in os.listdir(datadir):
    with open(datadir + datafilename, "r") as f:
        r = f.readlines()
        if "cost" in r[-2] and verifysize("44",r):
            for line_r in r:
...
plt.xlabel(r"Adiabatic Evolution Time $T_A$")
plt.ylabel(r"Overlap Error $\varepsilon$")
plt.scatter(adiabatic_times,epsilons,color="green")
plt.plot(x_eval_exp_fit, y_evaled_exp_fit,linewidth=3,color="red",label="Exponential Fit")
plt.plot(x_eval_poly_fit, y_evaled_poly_fit,linewidth=3,color="orange",label="Polynomial Fit")
